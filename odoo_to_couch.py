...
def get_dependences(complete_name):
    ...
    for field in register.read()[0].keys():
        ...
        if field == 'res_id' and register._name == 'mail.message':
            # Case it's an chatter model, it's not ane many2many but
            # it's important to be in order
            dependences.append(odoo.env[register.model].browse(
                field_value).get_xml_id().values()[0])
            continue
